...
#### 8.1.2. 処理の主体が属さない IdP からの連携先仲介レスポンス例
...
関連するアカウントが処理の主体のみ、かつ、連携先 TA に発行されたアクセストークンが有効である間は、上記プロトコルを省いても良い。
この場合、まず連携先 TA から連携元 TA へのレスポンスの際に、Set-Cookie ヘッダでセッションを宣言する必要がある。

|Cookie 名|値|
|:--|:--|
|Edo-Cooperation|セッション ID|

連携元 TA は、後の処理要請リクエストの際に、このセッションを Cookie ヘッダで宣言する。
